...
export const updateServings = function (newServings) {
  state.recipe.ingredients.forEach(ingredient => {
    ...
    // new quanitity = old quantity * new servings/ old servings
  });
...
// clear bookmarks storage - only for development
    ...
    const ingredients = Object.entries(newRecipe)
      ...
      .map(ingredient => {
        ...
        if (ingredientArr.length !== 3)
          throw new Error(
            'Wrong ingredients format - please use the specified format: Quantity,Unit,Description'
          );
